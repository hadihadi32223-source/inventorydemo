import { createMockRecord, deleteMockRecord, disableMockRecord, getMockData, updateMockRecord, type MockDataKey } from './mockData';
...
function fallbackResponse(path: string, options?: RequestInit) {
  ...
  if (options?.method === 'POST') {
    return createMockRecord(key, parseRequestBody(options));
  }
...
async function request(path: string, options?: RequestInit) {
  ...
  try {
    ...
    return await parseJsonResponse(res, path);
  } catch (error) {
    console.info(`Using local WMS mock data for ${path}`, error);
    return fallbackResponse(path, options);
  }
}
