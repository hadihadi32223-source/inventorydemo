...
export default function LowStock() {
      ...
      const lowRows: LowStockRow[] = currentStock
        .map((stock: any) => {
          ...
          return {
            ...
            sku: String(stock.sku ?? item.sku ?? ''),
            barcode: String(stock.barcode ?? item.barcode ?? ''),
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      const matchesSearch = !q || [row.item_name, row.sku, row.barcode, row.warehouse, row.location]
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded shadow-sm">
        <div className="px-4 py-3 border-b border-slate-100 flex flex-wrap items-center gap-3">
          <SearchBox value={search} onChange={setSearch} placeholder="بحث بالصنف، SKU، Barcode، المستودع أو الموقع" />
