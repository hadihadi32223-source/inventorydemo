...
import { Save, CheckCircle, Printer, X, Plus, Trash2, Edit, FilePlus, FileDown, RotateCcw } from 'lucide-react';
import Toolbar from '../components/Toolbar';
import { api } from '../lib/api';
import { cancelMessage, exportPdfPlaceholder, resetFormMessage, safePrint } from '../lib/uiActions';
...
export default function Transfer() {
  const [docNumber, setDocNumber] = useState('TR-2025-0001');
  ...
  useEffect(() => {
    Promise.all([api.warehouses(), api.locations()]).then(([w, l]) => { setWarehouses(w); setLocations(l); });
  }, []);

  const addLine = () => {
    setLines((prev) => [...prev, { id: Date.now(), item_name: '', qty: 1, unit: 'قطعة', from_location: '', to_location: '', notes: '' }]);
  };
  ...
  const saveDoc = async (status: string) => {
    try {
      ...
      alert(`تم حفظ التحويل بالحالة: ${status === 'approved' ? 'معتمد' : 'مسودة'}`);
    } catch (err) { alert((err as Error).message); }
  };

  return (
    <div className="space-y-4">
      <Toolbar title="التحويل بين المستودعات">
        <button onClick={() => saveDoc('draft')} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-300 bg-white text-slate-700 text-sm font-medium rounded hover:bg-slate-50"><Save className="w-4 h-4" />حفظ كمسودة</button>
        <button onClick={() => saveDoc('approved')} className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded"><CheckCircle className="w-4 h-4" />اعتماد</button>
        <button onClick={() => safePrint('تحويل بين المستودعات')} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-300 bg-white text-slate-700 text-sm font-medium rounded hover:bg-slate-50"><Printer className="w-4 h-4" />طباعة</button>
        ...
        <button onClick={() => exportPdfPlaceholder('تحويل بين المستودعات')} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-300 bg-white text-slate-700 text-sm font-medium rounded hover:bg-slate-50"><FileDown className="w-4 h-4" />PDF</button>
      </Toolbar>

      <div className="bg-white border border-slate-200 rounded shadow-sm">
        <div className="px-4 py-3 border-b border-slate-100 bg-slate-50 text-sm font-semibold text-slate-700">بيانات التحويل</div>
        <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">من المستودع</label>
            <select value={fromWarehouse} onChange={(e) => setFromWarehouse(e.target.value)} className="w-full border border-slate-300 rounded px-3 py-2 text-sm">
              <option value="">اختر</option>
              {warehouses.map((w) => <option key={w.id} value={w.name}>{w.name}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">إلى المستودع</label>
            <select value={toWarehouse} onChange={(e) => setToWarehouse(e.target.value)} className="w-full border border-slate-300 rounded px-3 py-2 text-sm">
              <option value="">اختر</option>
              {warehouses.map((w) => <option key={w.id} value={w.name}>{w.name}</option>)}
            </select>
          </div>
      ...
      <div className="bg-white border border-slate-200 rounded shadow-sm">
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          <span className="text-sm font-semibold text-slate-700">أصناف التحويل</span>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-700">
                ...
                <th className="text-right px-2 py-2 border-b border-slate-200 font-semibold text-xs w-44">موقع الإيداع</th>
              ...
              {lines.map((line) => (
                <tr key={line.id} className="border-b border-slate-100">
                  ...
                  <td className="px-2 py-1.5">
                    <select value={line.from_location} onChange={(e) => updateLine(line.id, 'from_location', e.target.value)} className="w-full border border-slate-300 rounded px-2 py-1 text-sm">
                      <option value=""></option>
                  ...
                  <td className="px-2 py-1.5">
                    <select value={line.to_location} onChange={(e) => updateLine(line.id, 'to_location', e.target.value)} className="w-full border border-slate-300 rounded px-2 py-1 text-sm">
                      <option value=""></option>
                      {locations.map((l) => <option key={l.id} value={`${l.warehouse} / ${l.zone}-${l.aisle}-${l.rack}-${l.level}-${l.bin}`}>{`${l.warehouse} / ${l.zone}-${l.aisle}-${l.rack}-${l.level}-${l.bin}`}</option>)}
                    </select>
                  </td>
