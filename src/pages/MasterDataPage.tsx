...
import { Edit, Eye, FileDown, Plus, Power, Printer, Search, Trash2 } from 'lucide-react';
    ...
    })),
    {
      ...
      render: (row: MasterRecord) => (
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => safePrint(`${config.title} - ${row.id}`)} className="p-1 rounded hover:bg-slate-100 text-slate-600" title="طباعة">
  ...
  return (
    <div className="space-y-4">
      <Toolbar title={config.title}>
        ...
        <button onClick={() => safePrint(config.title)} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-300 bg-white text-slate-700 text-sm font-medium rounded hover:bg-slate-50">
          <Printer className="w-4 h-4" />
          طباعة
        </button>
        <button onClick={() => exportCsv(config.title, filtered as Record<string, unknown>[])} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-300 bg-white text-slate-700 text-sm font-medium rounded hover:bg-slate-50">
...
export const configs = {
  ...
  manufacturers: {
    ...
    columns: [
      ...
      { key: 'country', header: 'البلد', width: '140px' },
      { key: 'notes', header: 'ملاحظات', width: 'auto' },
      { key: 'status', header: 'الحالة', width: '100px' },
    ],
  },
