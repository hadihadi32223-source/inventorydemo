    ...
    { key: 'last_login_at', header: 'آخر دخول', width: '180px', render: (row: AuthUser) => row.last_login_at ? new Date(row.last_login_at).toLocaleString('ar-SA') : '-' },
    {
      ...
      render: (row: AuthUser) => (
        <div className="flex items-center gap-2">
          <select
            value={row.role}
            onChange={(e) => { updateAuthUser(row.id, { role: e.target.value as RoleKey }); refresh(); }}
          ...
          <button
            ...
            onClick={() => { if (confirm('هل تريد تعطيل هذا المستخدم؟')) { disableAuthUser(row.id); refresh(); } }}
          >
          ...
          <button
            ...
            disabled={row.id === user?.id}
            onClick={() => { if (confirm('هل تريد حذف هذا المستخدم من النسخة المحلية؟')) { deleteAuthUser(row.id); refresh(); } }}
          >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded p-4 shadow-sm text-xs text-slate-500 leading-relaxed">يمكن تعطيل المستخدمين للحفاظ على سجل العمليات، ويمكن حذف المستخدمين التجريبيين من النسخة المحلية عند الحاجة. عدد الأدوار المستخدمة: <strong>{roleCount}</strong></div>
      </div>
