...
export default function InventoryCount() {
  ...
  const saveDoc = async (status: string) => {
    try {
      ...
      alert(`تم حفظ الجرد بالحالة: ${status === 'approved' ? 'معتمد' : 'مسودة'}`);
  ...
  return (
    <div className="space-y-4">
      <Toolbar title="الجرد">
        ...
        <button onClick={() => safePrint('الجرد')} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-300 bg-white text-slate-700 text-sm font-medium rounded hover:bg-slate-50"><Printer className="w-4 h-4" />طباعة</button>
      ...
      <div className="bg-white border border-slate-200 rounded shadow-sm">
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select value={warehouse} onChange={(e) => setWarehouse(e.target.value)} className="w-full border border-slate-300 rounded px-3 py-2 text-sm">
