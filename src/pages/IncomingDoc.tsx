...
export default function IncomingDoc() {
  ...
  const saveDoc = async (status: string) => {
    try {
      ...
      alert(`تم حفظ المستند بالحالة: ${status === 'approved' ? 'معتمد' : 'مسودة'}`);
  ...
  return (
    <div className="space-y-4">
      <Toolbar title="مستند وارد">
        ...
        <button onClick={() => safePrint('مستند وارد')} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-300 bg-white text-slate-700 text-sm font-medium rounded hover:bg-slate-50"><Printer className="w-4 h-4" />طباعة</button>
