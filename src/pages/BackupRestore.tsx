...
export default function BackupRestore() {
  ...
  const createBackup = async () => {
    await api.createBackup({ backup_date: new Date().toISOString(), backup_path: 'C:/ArabicWms/Backups/manual-backup.db', created_by: 'مدير النظام', status: 'success', notes: 'نسخة من الواجهة' });
    await load();
  ...
  return (
    <div className="space-y-4">
      <Toolbar title="النسخ الاحتياطي والاستعادة">
        ...
        <button onClick={() => showToast('مسار النسخ الاحتياطي: C:/ArabicWms/Backups')} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-300 bg-white text-slate-700 text-sm font-medium rounded hover:bg-slate-50"><FolderOpen className="w-4 h-4" />فتح المجلد</button>
      </Toolbar>
