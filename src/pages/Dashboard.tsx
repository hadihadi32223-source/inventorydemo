...
export default function Dashboard() {
  const [stats, setStats] = useState<Stats>({ totalItems: 0, totalWarehouses: 0, totalAvailableQty: 0, lowStock: 0, expiredOrNear: 0, incomingToday: 0, outgoingToday: 0, pendingTransfers: 0 });
  ...
  useEffect(() => {
    async function load() {
      try {
        const [items, warehouses, currentStock, movements, incoming, outgoing, transfers] = await Promise.all([
          api.items(),
          api.warehouses(),
          api.currentStock(),
          api.stockMovements(),
        ...
        setStats({
          ...
          pendingTransfers: transfers.filter((doc: any) => doc.status === 'draft' || doc.status === 'pending').length,
        });
  ...
  const operationalNote = useMemo(() => {
    if (stats.lowStock > 0) return 'يوجد أصناف تحت الحد الأدنى. راجع اقتراحات إعادة التوريد قبل اعتماد أي صادر جديد.';
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard title="عدد الأصناف" value={stats.totalItems} icon={<Package className="w-6 h-6" />} color="teal" />
        <StatCard title="عدد المستودعات" value={stats.totalWarehouses} icon={<Warehouse className="w-6 h-6" />} color="blue" />
        <StatCard title="إجمالي الكميات المتاحة" value={stats.totalAvailableQty} icon={<ClipboardCheck className="w-6 h-6" />} color="teal" />
        <StatCard title="أصناف تحت الحد الأدنى" value={stats.lowStock} icon={<AlertTriangle className="w-6 h-6" />} color="amber" />
        <StatCard title="أصناف قريبة/منتهية" value={stats.expiredOrNear} icon={<Bell className="w-6 h-6" />} color="red" />
        <StatCard title="مستندات وارد اليوم" value={stats.incomingToday} icon={<ArrowDownLeft className="w-6 h-6" />} color="teal" />
        <StatCard title="مستندات صادر اليوم" value={stats.outgoingToday} icon={<ArrowUpRight className="w-6 h-6" />} color="blue" />
        <StatCard title="تحويلات قيد الانتظار" value={stats.pendingTransfers} icon={<ArrowLeftRight className="w-6 h-6" />} color="amber" />
      </div>
