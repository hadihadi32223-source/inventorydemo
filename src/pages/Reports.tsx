...
const reportCards: ReportCard[] = [
  ...
  { key: 'warehouseMovement', title: 'تقرير حركة مستودع', icon: Warehouse, desc: 'كل الوارد والصادر والتحويلات داخل مستودع معين' },
  ...
  { key: 'transfers', title: 'تقرير التحويلات', icon: BarChart3, desc: 'كل التحويلات بين المستودعات ومواقع التخزين' },
...
export default function Reports() {
  ...
  const buildReport = async (report: ReportCard) => {
    ...
    try {
      ...
      if (report.key === 'transfers') rows = await api.transfers();
  ...
  const columns = useMemo(() => {
    ...
    const preferredKeys = Object.keys(headerMap).filter((key) => key in reportRows[0]);
    const extraKeys = Object.keys(reportRows[0]).filter((key) => !(key in headerMap) && typeof reportRows[0][key] !== 'object');
    ...
    <div className="space-y-4">
      <Toolbar title="التقارير">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => reportRows.length ? safePrint(selectedReport?.title ?? 'تقرير') : showToast('افتح التقرير أولًا قبل الطباعة.')} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-300 bg-white text-slate-700 text-sm font-medium rounded hover:bg-slate-50"><Printer className="w-4 h-4" />طباعة</button>
          <button onClick={() => exportCsv(selectedReport?.title ?? 'التقارير', reportRows.length ? reportRows : reportCards.map((r) => ({ title: r.title, description: r.desc })))} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-300 bg-white text-slate-700 text-sm font-medium rounded hover:bg-slate-50"><Download className="w-4 h-4" />تصدير</button>
        </div>
      </Toolbar>
        ...
        <div className="report-preview-header px-4 py-3 flex flex-wrap items-center gap-3 justify-between">
          ...
          {selectedReport && (
            <div className="flex gap-2">
              <button onClick={() => safePrint(selectedReport.title)} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-300 bg-white text-slate-700 text-sm font-medium rounded hover:bg-slate-50"><Printer className="w-4 h-4" />طباعة التقرير</button>
              <button onClick={() => exportCsv(selectedReport.title, reportRows)} className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-300 bg-white text-slate-700 text-sm font-medium rounded hover:bg-slate-50"><Download className="w-4 h-4" />تصدير CSV</button>
            </div>
          )}
        </div>
        {selectedReport ? (
