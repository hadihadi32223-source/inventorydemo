...
export default function Locations() {
  ...
  const fetch = async () => {
    try { setLocations(await api.locations()); } finally { setLoading(false); }
  };
  useEffect(() => { fetch(); }, []);
          ...
          <div className="bg-white rounded shadow-xl w-full max-w-2xl">
            ...
            <form onSubmit={handleSave} className="p-4 grid grid-cols-2 gap-4">
              <div>
                ...
                <input name="warehouse" defaultValue={editing?.warehouse} required className="w-full border border-slate-300 rounded px-3 py-2 text-sm" />
              </div>
              <div>
